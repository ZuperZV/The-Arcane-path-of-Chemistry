...
public class HeaterMenu extends AbstractContainerMenu {
    ...
    public HeaterMenu(int containerId, Player player, BlockPos pos) {
        super(ModMenuTypes.HEATER_MENU.get(), containerId);
        HeaterBlockEntity HeaterBlockEntity;
        this.pos = pos;

        if (player.level().getBlockEntity(pos) instanceof HeaterBlockEntity blockentity) {
            HeaterBlockEntity = blockentity;
            this.addDataSlots(blockentity.data);

            addSlot(new SlotItemHandler(blockentity.getInputItems(), 0, 39, 20));

        } else {
            HeaterBlockEntity = null;
        ...
        this.blockentity = HeaterBlockEntity;
    }

    @Override
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(blockentity.getLevel(), blockentity.getBlockPos()), player, ModBlocks.HEATER.get());
    }
